...
| `--mass-tolerance` is the tolerance the run declares, echoed in `run`, and every case reports the mass of its mixture, exactly the library's | L0, L1, L2 (`CommandLineTests`, `OutputDocumentTests.TheMassToleranceIsEchoedAndEveryCaseReportsTheMassOfItsMixture`, `ExitCodeTests.TheMassToleranceOptionIsTheToleranceTheRunDeclares`, `LibraryEqualityTests`) | ✅ |
