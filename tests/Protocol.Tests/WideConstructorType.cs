...
/// so that the type's namespace is <c>Node.Namespace</c> (equal to <c>Node.AssemblyName</c> for a node with its own project,
/// the only definition a project-less child node has); <see cref="SimpleName"/> is the type's own simple name, unqualified
...
/// </summary>
internal readonly record struct WideConstructorType(Node Node, string NestingPath, string SimpleName);
