...
{
    /// <summary>Mole fractions below this are not compared: their logarithms are not converged to the same digits.</summary>
    ...
    public static readonly IReadOnlyDictionary<string, (double Relative, string Derivation)> Entries = new Dictionary<string, (double, string)>(StringComparer.Ordinal)
    {
        ...
        ["moleFractionAfterDifferentSteps"] = (1e-9, "Mole fractions at a station where one accelerator took one polish step more: the step moves ln n_j by dln n + (H_j/RT) dln T + sum_i a_ij dpi_i with every term of the order of the 1e-11 threshold, so a species with |H_j/RT| and sum_i a_ij of a few tens moves by a few 1e-10 (measured 3.3e-10 on HO2)."),
    ...
    public static double MoleFractionRelative(bool sameSteps) => Entries[sameSteps ? "moleFraction" : "moleFractionAfterDifferentSteps"].Relative;
