...
/// except <see cref="PlateauTests"/>, which holds its own instance. Held as the single <see cref="Shared"/> instance
/// rather than through <c>ICollectionFixture&lt;T&gt;</c>: xUnit requires a class fixture's consuming constructor to
/// be the class's single public constructor, which would force this internal-only helper public for no reason a
/// consumer outside this node has (CA1515, confirmed empirically: a public <c>CpuFixture</c> with a
/// <c>[CollectionDefinition]</c> wrapper fails both CA1515 and CA1711 on a clean build of this project, regardless of
/// whether a consuming class injects it). The classes that shared one <c>CpuFixture</c> through the "cpu"
...
{
    /// <summary>The xUnit collection name every consuming class but <see cref="PlateauTests"/> is tagged with.</summary>
