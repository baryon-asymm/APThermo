...
public sealed class AbsentElementTests
{
    ...
    public void ATableWithTheSpeciesOfAbsentElementsGivesTheSameBits(string name, string other)
    {
        var c = TransportHost.LoadRocket(name);
        var o = TransportHost.LoadRocket(other);
        var (own, ownTransport) = TransportHost.TablesOf(CpuFixture.Shared, c);
        ...
        var union = SpeciesTable.Build(CpuFixture.Shared.Database, elements, products);
        var unionTransport = TransportTable.Build(CpuFixture.Shared.Transport, union);
        Assert.True(union.GasCount > own.GasCount, "the other case must bring gaseous species");

        using var ownSpecies = SpeciesTableBuffers.Upload(CpuFixture.Shared.Accelerator, own);
        using var ownBuffers = TransportTableBuffers.Upload(CpuFixture.Shared.Accelerator, ownTransport);
        using var unionSpecies = SpeciesTableBuffers.Upload(CpuFixture.Shared.Accelerator, union);
        using var unionBuffers = TransportTableBuffers.Upload(CpuFixture.Shared.Accelerator, unionTransport);
        ...
        foreach (var station in stations)
        {
            ...
            var expected = TransportHost.Evaluate(CpuFixture.Shared.Accelerator, ownSpecies, ownBuffers, temperature, TransportHost.MolesOf(own, station));
            var actual = TransportHost.Evaluate(CpuFixture.Shared.Accelerator, unionSpecies, unionBuffers, temperature, TransportHost.MolesOf(union, station));
